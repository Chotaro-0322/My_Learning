...
class MyFrame(rtmgr.MyFrame):
	...
	def save_param_yaml(self):
		...
		for (name, pdic) in self.load_dic.items():
			if pdic and pdic != {}:
				...
				pdic = pdic.copy()
				for k in list(pdic): # before pdic.key()
					if k in no_saves:
						del pdic[k]
				save_dic[name] = pdic
	...
	def exec_time_callback(self, msg, temps):
		key, attr = temps
	...
	def OnTipTimer(self, event):
		if getattr(self, 'tip_info', None):
			...
			wx.TipWindow(tree, text, maxLength=w)
	...
	def rosparam_set(self, pdic, prm):
		...
		for var in prm.get('vars', []):
			...
			if rosparams is None:
				cmd = [ 'rosparam', 'list' ]
				rosparams = subprocess.check_output(cmd).strip().decode().split('\n')
			nm = rosparam
	...
	def mem_kb_info(self):
		lines = subprocess.check_output('cat /proc/meminfo', shell=True).strip().decode().split(os.linesep)
		#cvt = lambda (k, v): ( k.replace(':', ''), int(v) )
		cvt = lambda temps: ( temps[0].replace(':', ''), int(temps[1]) )
		d = dict( map( lambda s: cvt( list(filter( lambda s: s!='kB', s.split() ) )), lines) )
	...
	def refresh_topics_list(self):
		lst = subprocess.check_output([ 'rostopic', 'list' ]).strip().decode().split('\n')
	...
	def create_tree(self, parent, items, tree, item, cmd_dic):
		...
		if tree is None:
			...
			tree.AcceptsFocus = MethodType(lambda self: False, tree)
...
class MyDialogROSbagRecord(rtmgr.MyDialogROSbagRecord):
	...
	def refresh(self):
		lst = [ 'all' ] + subprocess.check_output([ 'rostopic', 'list' ]).strip().decode().split('\n')
		panel = self.panel_1
...
def str_to_rosval(s, type_str, def_ret=None):
	...
	} # change before 'int64':long , 'uint64':long,
